/**
 * CivicPulse — Landing Footer
 */
import { Link } from 'react-router-dom';

/* Simple inline SVG social icons — Lucide doesn't ship brand icons */
const GithubIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/>
    <path d="M9 18c-4.51 2-5-2-7-2"/>
  </svg>
);
const TwitterIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"/>
  </svg>
);
const LinkedinIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/>
    <rect width="4" height="12" x="2" y="9"/>
    <circle cx="4" cy="4" r="2"/>
  </svg>
);

export default function LandingFooter() {
  const scrollTo = (id) => (e) => {
    e.preventDefault();
    document.querySelector(id)?.scrollIntoView({ behavior: 'smooth' });
  };

  return (
    <footer className="lp-footer">
      <div className="lp-footer__inner">
        {/* Brand */}
        <div className="lp-footer__brand">
          <div className="lp-nav__logo">
            <span className="lp-logo-civic">Civic</span>
            <span className="lp-logo-dot">●</span>
            <span className="lp-logo-pulse">Pulse</span>
          </div>
          <p className="lp-footer__tagline">Listen before they have to.</p>
          <div className="lp-footer__social">
            <a href="#" aria-label="GitHub" onClick={(e) => e.preventDefault()}><GithubIcon /></a>
            <a href="#" aria-label="Twitter" onClick={(e) => e.preventDefault()}><TwitterIcon /></a>
            <a href="#" aria-label="LinkedIn" onClick={(e) => e.preventDefault()}><LinkedinIcon /></a>
          </div>
        </div>

        {/* Platform Links */}
        <div className="lp-footer__col">
          <h4 className="lp-footer__heading">Platform</h4>
          <a href="#how-it-works" onClick={scrollTo('#how-it-works')}>How It Works</a>
          <a href="#signal-sources" onClick={scrollTo('#signal-sources')}>Signal Sources</a>
          <a href="#impact" onClick={scrollTo('#impact')}>Impact</a>
          <a href="#for-ngos" onClick={scrollTo('#for-ngos')}>For NGOs</a>
          <Link to="/login">Sign In</Link>
        </div>

        {/* Legal Links */}
        <div className="lp-footer__col">
          <h4 className="lp-footer__heading">Legal</h4>
          <Link to="/privacy">Privacy Policy</Link>
          <Link to="/terms">Terms of Service</Link>
          <Link to="/dpa">Data Processing Agreement</Link>
          <Link to="/dpdpa">DPDPA Compliance</Link>
        </div>
      </div>

      <div className="lp-footer__bottom">
        <span>&copy; 2026 CivicPulse. All rights reserved.</span>
        <span>Built for communities. Powered by AI.</span>
      </div>
    </footer>
  );
}
