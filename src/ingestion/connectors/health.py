"""
CivicPulse — Health Worker Logs Signal Connector

Aggregates frontline health worker field observations.
Intensity mapping:
  0.0-0.3: Routine activity levels
  0.3-0.6: Increased case reports
  0.6-1.0: Surge-level health concerns
"""

import logging
import os
from datetime import datetime, timezone
from typing import AsyncGenerator

import httpx

from anonymizer import anonymize_at_source
from base import BaseConnector, UnifiedSignal
from registry import register_connector

logger = logging.getLogger(__name__)

HEALTH_API_URL = os.environ.get("HEALTH_API_URL", "")
HEALTH_API_KEY = os.environ.get("HEALTH_API_KEY", "")


class HealthConnector(BaseConnector):
    """Connector for health worker observation logs (hourly batch)."""

    def __init__(self):
        super().__init__(source_name="health_worker_logs", signal_type="health")

    async def fetch(self) -> AsyncGenerator[UnifiedSignal, None]:
        if not HEALTH_API_URL:
            logger.error("HEALTH_API_URL not configured")
            return

        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(
                    f"{HEALTH_API_URL}/logs/aggregate",
                    headers={"Authorization": f"Bearer {HEALTH_API_KEY}"},
                )
                response.raise_for_status()
                data = response.json()

                for record in data.get("health_reports", []):
                    try:
                        anonymized = anonymize_at_source(record)
                        if not self.validate(anonymized):
                            continue
                        yield self.normalize(anonymized)
                    except Exception as e:
                        logger.error("Failed to process health record: %s", e)

        except httpx.HTTPStatusError as e:
            logger.error("Health API HTTP error %d: %s", e.response.status_code, e)
        except httpx.RequestError as e:
            logger.error("Health API request failed: %s", e)
        except Exception as e:
            logger.error("Unexpected error fetching health data: %s", e)

    def validate(self, raw: dict) -> bool:
        return "location_pin" in raw

    def normalize(self, raw: dict) -> UnifiedSignal:
        intensity = max(0.0, min(1.0, float(raw.get("intensity_score", 0.2))))
        return UnifiedSignal(
            source=self.source_name,
            location_pin=str(raw["location_pin"]),
            signal_type=self.signal_type,
            intensity_score=intensity,
            timestamp=raw.get("timestamp", datetime.now(timezone.utc).isoformat()),
            confidence=max(0.0, min(1.0, float(raw.get("confidence", 0.75)))),
        )


register_connector("health", HealthConnector)
