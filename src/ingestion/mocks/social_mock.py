"""CivicPulse — Mock Social Media Connector. Development-only."""
...
class SocialMockConnector(BaseConnector):
    def __init__(self):
        super().__init__(source_name="social_sentiment_mock", signal_type="social")
    ...
    async def fetch(self) -> AsyncGenerator[UnifiedSignal, None]:
        for s in self._load():
            try:
                ...
                logger.error("Mock social error: %s", e)
    ...
    def _load(self) -> list[dict]:
        try:
            ...
            if os.path.exists(p):
                with open(p) as f:
                    return [s for s in json.load(f) if s.get("signal_type") == "social"]
        except Exception:
            pass
        return [{"location_pin": f"WARD-DEL-{i:03d}", "signal_type": "social",
                 "intensity_score": random.uniform(0.1, 0.8), "confidence": random.uniform(0.5, 0.85)}
                for i in range(1, 11)]

register_mock_connector("social", SocialMockConnector)
