"""CivicPulse — Mock Utility Connector. Development-only."""
...
class UtilityMockConnector(BaseConnector):
    def __init__(self):
        super().__init__(source_name="utility_payments_mock", signal_type="utility")
    ...
    async def fetch(self) -> AsyncGenerator[UnifiedSignal, None]:
        for s in self._load():
            try:
                ...
                logger.error("Mock utility error: %s", e)
    ...
    def _load(self) -> list[dict]:
        try:
            ...
            if os.path.exists(p):
                with open(p) as f:
                    return [s for s in json.load(f) if s.get("signal_type") == "utility"]
        except Exception:
            pass
        return [{"location_pin": f"WARD-DEL-{i:03d}", "signal_type": "utility",
                 "intensity_score": random.uniform(0.1, 0.7), "confidence": random.uniform(0.6, 0.9)}
                for i in range(1, 11)]

register_mock_connector("utility", UtilityMockConnector)
